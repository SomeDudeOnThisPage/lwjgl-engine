...
void main()
{
  ...
  for (int k = 0; k < u_num_directional_lights; k++)
  {
    ...
    specular += dlight.col.xyz * (spec * u_material.specular);

    diffuse += (max(dot(normal, normalize(light_direction)), 0.0) * normalize(dlight.col.xyz) * normalize(u_material.diffuse));
  }
  ...
  for (int j = 0; j < u_num_point_lights; j++)
  {
    ...
    specular += light.col.xyz * (spec * u_material.specular) * attenuation;
    diffuse += max(dot(normal, normalize(light_direction)), 0.0) * normalize(light.col.xyz) * u_material.diffuse * attenuation;
  }
